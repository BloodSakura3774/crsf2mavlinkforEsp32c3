...
void startCrossfire(){                                                   //START THE SERIAL PORT
     ...
     CROSSFIRE_SERIAL.begin(CROSSFIRE_BAUD_RATE,SERIAL_8N1, 21, 20);     
     //csrfTimer.begin(runCrossfire, (REFRESH_INTERVAL*1000)); 
     csrfTimer.attach_ms(REFRESH_INTERVAL,runCrossfire);
}
...
void processCrossfireTelemetryData(uint8_t data)
{
  if (telemetryRxBufferCount == 0 && data != RADIO_ADDRESS) {
    if(crossfireDebug == 1){
        SerialX.print("[XF] address 0x%02X error ");
        SerialX.println(data);
    }
    return;
  }
  ...
  if (telemetryRxBufferCount == 1 && (data < 2 || data > TELEMETRY_RX_PACKET_SIZE-2)) {
    if(crossfireDebug == 1){
      SerialX.print("[XF] length 0x%02X error ");
      SerialX.println(data);  
    }  
  ...
  if (telemetryRxBufferCount < TELEMETRY_RX_PACKET_SIZE) {
    telemetryRxBuffer[telemetryRxBufferCount++] = data;
        ...
        if(crossfireDebug == 1){
            SerialX.print("[XF] array size error ");
            SerialX.println(telemetryRxBufferCount);    
        }    
    telemetryRxBufferCount = 0;
  }
...
void setTelemetryValue( uint16_t id, uint8_t subId, uint8_t index, uint8_t instance, int32_t value, uint32_t unit, uint32_t prec){
              ...
              if(crossfireDebug == 1){
                  ...
                  SerialX.print(" INDEX: ");
                  SerialX.print(index);
                  SerialX.print(" INSTANCE: ");
                  SerialX.print(instance);
                  SerialX.print(" VALUE ");
                  SerialX.print(value);
                  SerialX.print(" UNIT ");
                  SerialX.print(unit);
                  SerialX.print(" PRECISION ");
                  SerialX.print(prec);
                  SerialX.println(" ");
              }
